<template>
  <div class="ratingselect">
    <div class="rating-type">
      <span @click="select(2,$event)" class="block positive" :class="{'active':sType===2}">{{desc.all}}<span class="count">{{ratings.length}}</span></span>
    ...
    <div class="switch" @click="toggleContent" :class="{'on':oContent}">
    ...
    export default {
      ...
      methods:{
        ...
        toggleContent(event){
          ...
          this.$emit(
            'increment', 'onlyContent',this.oContent)
        }
      }
    }
</script>
